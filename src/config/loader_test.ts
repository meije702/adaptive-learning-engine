...
Deno.test("loadConfig - loads k8s-hybrid-cloud example successfully", async () => {
...
Deno.test("loadConfig - domain prerequisites are string arrays", async () => {
  const config = await loadConfig(EXAMPLE_DIR);
...
Deno.test("loadConfig - levels have correct id range", async () => {
  const config = await loadConfig(EXAMPLE_DIR);
...
Deno.test("loadConfig - throws ConfigError for missing directory", async () => {
  await assertRejects(
...
Deno.test("loadConfig - uses ALE_CONFIG_DIR env var", async () => {
