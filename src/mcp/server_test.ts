...
describe("MCP tool logic — Layer 2 integration (InMemoryTransport)", () => {
  ...
  describe("Observe tools", () => {
    ...
    it("get_progress returns all entries", async () => {
      ...
      const result = await client.callTool({ name: "get_progress", arguments: {} });
      const data = parse(result);
    ...
    it("get_progress returns single entry by domainId", async () => {
      ...
      const result = await client.callTool({ name: "get_progress", arguments: { domainId: "domain-a" } });
      const data = parse(result);
      assertExists(data);
    ...
    it("get_pending_answers returns only unanswered questions", async () => {
      const day = await repos.days.create({
        ...
        domainId: "domain-a", title: "Test", body: "Body",
      });
      ...
      const result = await client.callTool({ name: "get_pending_answers", arguments: {} });
      const data = parse(result);
    ...
    it("get_retention_due returns only past-due schedules", async () => {
      ...
      const result = await client.callTool({ name: "get_retention_due", arguments: {} });
      const data = parse(result);
      assertEquals(data.length, 1);
    ...
    it("get_scene_document returns scene and interaction log", async () => {
      ...
      const day = await repos.days.create({
        weekNumber: 1, dayOfWeek: 1, type: "theory",
        domainId: "domain-a", title: "T", body: "B", sceneDocument: scene,
      });

      const result = await client.callTool({ name: "get_scene_document", arguments: { dayContentId: day.id } });
      const data = parse(result);
  ...
  describe("Generate tools", () => {
    it("create_week_plan persists", async () => {
      const result = await client.callTool({
        ...
        arguments: { weekNumber: 1, domainId: "domain-a", isStretchWeek: false, summary: "Intro week" },
      });
      const data = parse(result);
    ...
    it("create_day_content with valid sceneDocument stores it", async () => {
      ...
      const result = await client.callTool({
        ...
        arguments: {
          weekNumber: 1, dayOfWeek: 1, type: "theory",
          domainId: "domain-a", title: "Theory Day", body: "Summary text",
          sceneDocument: scene,
        },
      });
      const data = parse(result);
    ...
    it("create_day_content with invalid sceneDocument returns validation errors", async () => {
      ...
      const result = await client.callTool({
        ...
        arguments: {
          weekNumber: 1, dayOfWeek: 1, type: "theory",
          domainId: "domain-a", title: "Bad", body: "Body",
    ...
    it("create_questions creates linked entities", async () => {
      const day = await repos.days.create({
        weekNumber: 1, dayOfWeek: 1, type: "practice_guided",
        domainId: "domain-a", title: "Practice", body: "Body",
      });
      ...
      const result = await client.callTool({
        ...
        arguments: {
          dayContentId: day.id,
          questions: [
            { domainId: "domain-a", sequence: 1, type: "scenario", body: "Scenario Q?", maxLevel: 3, deadline: new Date(Date.now() + 86400000).toISOString() },
            {
              domainId: "domain-a", sequence: 2, type: "multiple_choice", body: "MC Q?", maxLevel: 2,
    ...
    it("create_feedback without applyLevel does not update progress", async () => {
      const day = await repos.days.create({
        weekNumber: 1, dayOfWeek: 5, type: "assessment",
        domainId: "domain-a", title: "Assessment", body: "Body",
      });
      const [question] = await repos.questions.create([{
        dayContentId: day.id, domainId: "domain-a", sequence: 1,
        type: "open", body: "Open Q?", maxLevel: 4,
        deadline: new Date(Date.now() + 86400000).toISOString(),
      }]);
      const answer = await repos.answers.create({ questionId: question.id, body: "My answer" });
    ...
    it("create_feedback with applyLevel=true updates progress", async () => {
      ...
      await client.callTool({
        ...
        arguments: {
          answerId: answer.id, questionId: question.id,
          score: "correct", explanation: "Well done", suggestedLevel: 3,
          applyLevel: true, improvements: [],
        },
      });
    ...
    it("create_feedback with structured feedback fields persists them", async () => {
      ...
      const result = await client.callTool({
        ...
        arguments: {
          answerId: answer.id, questionId: question.id,
          score: "partial", explanation: "Decent", suggestedLevel: 2,
          applyLevel: false, improvements: ["be more specific"],
  ...
  describe("Calibration tools", () => {
    it("record_self_assessment computes delta for overestimate (-1)", async () => {
      ...
      await repos.feedback.create({
        answerId: answer.id, questionId: question.id,
        score: "incorrect", explanation: "Wrong",
        suggestedLevel: 1, applyLevel: false, improvements: [],
      });
    ...
    it("record_self_assessment computes delta for calibrated (0)", async () => {
      ...
      await repos.feedback.create({
        answerId: answer.id, questionId: question.id,
        score: "correct", explanation: "Good",
    ...
    it("get_calibration_summary returns summary", async () => {
      ...
      const result = await client.callTool({ name: "get_calibration_summary", arguments: {} });
      const data = parse(result);
      assertExists(data);
    });
  });
  ...
  describe("Intake tools", () => {
    it("start_intake creates session with config context", async () => {
      const result = await client.callTool({ name: "start_intake", arguments: {} });
      const data = parse(result);
    ...
    it("start_intake returns existing session if already started", async () => {
      const result1 = await client.callTool({ name: "start_intake", arguments: {} });
      const data1 = parse(result1);
      const sessionId = data1.session.id;

      const result2 = await client.callTool({ name: "start_intake", arguments: {} });
    ...
    it("send_intake_message stores message and advances phase", async () => {
      ...
      const result = await client.callTool({
        ...
        arguments: { content: "Tell me about yourself", phase: "profile_validation" },
      });
      const data = parse(result);
  ...
  describe("Gap Analysis tools", () => {
    it("get_gap_analysis returns per-phase analysis", async () => {
      ...
      const result = await client.callTool({ name: "get_gap_analysis", arguments: {} });
      const data = parse(result);

      assertEquals(data.phaseGaps.length, 2);
      assertExists(data.phaseGaps.find((p: { phaseId: number }) => p.phaseId === 1));
      assertExists(data.phaseGaps.find((p: { phaseId: number }) => p.phaseId === 2));
    });

    it("get_gap_analysis filters by phaseId", async () => {
      await repos.progress.put("domain-a", { level: 3, source: "assessment" });

      const result = await client.callTool({ name: "get_gap_analysis", arguments: { phaseId: 1 } });
      const data = parse(result);
  ...
  describe("Wellbeing tools", () => {
    ...
    it("set_wellbeing_status sets returning", async () => {
      // First pause
      await client.callTool({ name: "set_wellbeing_status", arguments: { status: "paused" } });
      // Then return
      const result = await client.callTool({
        name: "set_wellbeing_status",
  ...
  describe("Scheduling tools", () => {
    it("get_scheduled_tasks returns manifest and schedule", async () => {
      const result = await client.callTool({ name: "get_scheduled_tasks", arguments: {} });
      const data = parse(result);
